...
//Check how use typealias Response<D> = ApolloResponse<D> in getResult Function
//1. We can two getResult  = such getResultFromGraphQl and getResultFromRetrofit
//TODO List
/*   1. check for context for showing error:-
        -> we need activity context for show and snackbar
        -> check for custom snackbar UI with image loading in it using glide?:Default
    2. title, image, type of error to shown and isShow(to show or not) come from the backend in error response
    ...
    6. if is_show not come from backend the is_Show will be true and if false then we don't show the error on UI
    7. Client will able handle some action with snackbar
    8.IMPORTANT NOT: When we got exception it will not give us error_response instead it will give the exception then we have to show
                     default Snackbar with message
    9. Also design custom UI for Snackbars

    TODO Important
    1. Change ErrorResponse data class because it will contain (type_of_error, is_shown, image_url,error_message)
  ...
  3. think to change this above data class with more param or add one more data class for ErrorResponseFromBackend(which come from the backend)
* */
...
//also design the custom UI for snack bar and toast
...
//sealed class HttpErrors<out T> : Resource<T>() {
//    data class ResourceForbidden<T>(val exception: T) : HttpErrors<T>()
